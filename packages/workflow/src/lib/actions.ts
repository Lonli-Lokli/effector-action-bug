...
type RemoveDollarPrefix<Key extends string> = Key extends `$${infer K}`
  ? K
  : Key;
...
type GetClockValue<Clc extends ClockShape<any>> = [Clc] extends [Unit<any>]
  ? UnitValue<Clc>
  : GetTupleWithoutAny<Clc>;
type GetSourceValue<Src extends Store<any> | SoureShape> = Src extends Store<
  infer Value
>
  ? Value
  : {
      [K in RemoveDollarPrefix<keyof Src & string>]: UnitValue<
        Src[keyof Src & IsNever<K & keyof Src, `$${K}`, K>]
      >;
    };
...
const getStoreForPrevValueKey = (storeName: string) =>
  `__${storeName}_prevValue__`;
...
>(
  ...
  const target: TargetShape = is.unit(passedTarget)
    ? { [getUnitTargetKey()]: passedTarget }
    : { ...passedTarget };
  ...
  sample({
    ...
    fn: (source, clock) => {
      ...
      const fnTarget = is.unit(passedTarget)
        ...
        : Object.fromEntries(
            Object.entries(passedTarget).map(([unitName, unit]) => [
              unitName,
...
const removeDollarPrefix = (sourceShape: SoureShape): SoureShape => {
  return Object.fromEntries(
    Object.entries(sourceShape).map(([key, store]) => [
